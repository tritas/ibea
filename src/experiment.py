...
def coco_optimize(solver, fun, max_evals, max_runs=1e9):
    ...
    for restarts in range(int(max_runs)):
        ...
        if solver.__name__ in ("random_search", "ibea",):
            ...
            solver(fun, x0, iter=1 + remaining_evals / fun.dimension,
                   iprint=-1)
        # HERE: Implement other function calls
        else:
...
##############################################################################
ibea_object = IBEA(alpha=150, pr_mut=0.01, pr_x=1.0, n_offspring=20, var=5.0)
SOLVER = ibea_object.ibea
#SOLVER = my_solver # fmin_slsqp # SOLVER = cma.fmin
suite_name = "bbob-biobj"
#suite_name = "bbob"
suite_instance = "year:2016"
suite_options = "dimensions: 2,5"  # "dimensions: 2,3,5,10,20 "  # if 40 is not desired
